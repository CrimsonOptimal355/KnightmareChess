...
void loadTexture(std::unordered_map<char, sf::Texture> &textures, char key, const std::string &path)
{
...
int main()
{
    ...
    sf::RenderWindow window(
        sf::VideoMode({static_cast<unsigned int>(windowWidth), static_cast<unsigned int>(windowHeight)}), "Chess");
    ...
    gameOverPanel.setPosition({windowWidth / 2.f - 250.f, windowHeight / 2.f - 150.f});
    ...
    rematchButton.setPosition({windowWidth / 2.f - 210.f, windowHeight / 2.f + 40.f});
    ...
    rematchText.setPosition({windowWidth / 2.f - 180.f, windowHeight / 2.f + 55.f});
    ...
    while (window.isOpen())
    {
        ...
        while (auto event = window.pollEvent())
        {
            ...
            if (const auto *mouseClick = event->getIf<sf::Event::MouseButtonPressed>())
            {
                ...
                {
                    int clickedCol = static_cast<int>((mouseClick->position.x - offsetX) / tileSize);
                    int clickedRow = static_cast<int>((mouseClick->position.y - offsetY) / tileSize);
                    ...
                    if (clickedCol >= 0 && clickedCol < 8 && clickedRow >= 0 && clickedRow < 8)
                    {
                        if (clickCount == 0)
                        {
                            ...
                            if (board[clickedRow][clickedCol] != '.')
                            {
                                ...
                                if ((whiteTurn && isBlack(selectedPiece)) || (!whiteTurn && isWhite(selectedPiece)))
                                {
                                    continue;
                                }
                                ...
                                for (int endRow = 0; endRow < 8; ++endRow)
                                {
                                    for (int endCol = 0; endCol < 8; ++endCol)
                                    {
                                        ...
                                        if (((isWhite(sourcePiece) && isWhite(targetPiece)) || (isBlack(sourcePiece) && isBlack(targetPiece))) ||
                                            !isValidMove(selectedRow, selectedCol, endRow, endCol, sourcePiece, targetPiece))
                                        {
                                            continue;
                                        }

                                        Move move{selectedRow, selectedCol, endRow, endCol, sourcePiece, targetPiece};
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                bool validMove = isValidMove(move.startRow, move.startCol, move.endRow, move.endCol, sourcePiece, targetPiece);
                                bool castlingMove = isValidCastling(move.startRow, move.startCol, move.endRow, move.endCol, sourcePiece,
                                ...
                                if (validMove)
                                {
                                    ...
                                    bool isCastlingAttempt = castlingMove && (sourcePiece == 'K' || sourcePiece == 'k') && move.startCol == 4;
        ...
        for (int row = 0; row < 8; row++)
        {
            for (int col = 0; col < 8; col++)
            {
                square.setPosition(sf::Vector2f(offsetX + col * tileSize, offsetY + row * tileSize));
                // Alternate Colouring
                if ((row + col) % 2 == 0)
                {
        ...
        for (int col = 0; col < 8; col++)
        {
            ...
            fileText.setPosition({offsetX + col * tileSize + tileSize - 15.f, offsetY + totalGridHeight - 28.f});
        ...
        {
            boardbox.setPosition({offsetX + selectedCol * tileSize, offsetY + selectedRow * tileSize});
        ...
        {
            // Previous square
            boardbox1.setPosition({offsetX + lastStartCol * tileSize, offsetY + lastStartRow * tileSize});

            window.draw(boardbox1);

            // New square
            boardbox1.setPosition({offsetX + lastEndCol * tileSize, offsetY + lastEndRow * tileSize});
        ...
        if (isKingInCheck(true))
        {
            ...
            boardbox2.setPosition({offsetX + kingPos.y * tileSize, offsetY + kingPos.x * tileSize});
        ...
        if (isKingInCheck(false))
        {
            ...
            boardbox2.setPosition({offsetX + kingPos2.y * tileSize, offsetY + kingPos2.x * tileSize});
        ...
        moveDot.setFillColor(
            sf::Color(80, 80, 80, 180));

        for (const auto &move : legalMoves)
        {
            moveDot.setPosition(
                {offsetX + move.y * tileSize + 47.f,
        ...
        for (int row = 0; row < 8; row++)
        {
            for (int col = 0; col < 8; col++)
            {
                ...
                sprite.setPosition({offsetX + col * tileSize, offsetY + row * tileSize});

                sprite.setScale({tileSize / textures[piece].getSize().x, tileSize / textures[piece].getSize().y});
        ...
        if (!isAnyLegalMove(whiteTurn))
        {
            ...
            if (isKingInCheck(whiteTurn))
                finalText.setString(whiteTurn ? "CHECKMATE - Black Wins" : "CHECKMATE - White Wins");
            if (!isKingInCheck(whiteTurn))
                finalText.setString("Draw by STALEMATE");

            finalText.setPosition({historyX, historyY + moveHistory.size() * 20.f + 40.f});
